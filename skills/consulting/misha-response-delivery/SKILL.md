...
- Pitfall: a Telegram reply wrapper contains a visibly truncated older message, and the assistant treats that truncation as a blocker for the live ask (`сообщение оборвалось`, `пришли полный список`) even though the user explicitly asked to judge the situation `как есть`.
  Fix: in reply-wrapper mode, distinguish two layers: (1) the quoted fragment is only context and may be cut off; (2) the live ask after the wrapper is the contract. If the live ask is `оцени`, `пересчитай`, `прикинь`, `что думаешь`, or another top-down judgment request, work from the visible facts in the quote plus clearly stated assumptions. Do not reopen the turn with clarification just because the quoted context was truncated, unless the missing tail changes the answer so materially that even a bounded estimate would be misleading. In estimate tasks, prefer `оценка по видимой части + что остаётся под вопросом` over asking the user to reconstruct the old quoted message.
