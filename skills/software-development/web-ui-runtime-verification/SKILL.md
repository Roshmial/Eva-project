...
- In policy/source-routing flows, preserve the distinction between `group present with []` and `group absent` during backend normalization. Explicit empty arrays coming from the frontend are valid state, not a signal to restore defaults.
