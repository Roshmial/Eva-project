...
## Async/background migration note

When a previously synchronous chat or message endpoint moves to background-job processing, do not keep old tests that assert the final assistant payload directly in the POST response. Split verification into phases:

1. POST returns a pending or processing placeholder plus a task handle.
2. Tests explicitly drive the worker path, or poll deterministically in integration tests.
3. Final assertions re-read the thread or task result and validate the completed assistant message there.

Mirror the same flow in UI/runtime verification: submit, show processing state, re-fetch thread or task status, then assert the final artifact.

Critical backend pitfall: when the worker assembles transcript history for the model, exclude the current pending assistant placeholder from the downstream transcript. Otherwise the model can see its own processing stub as the latest assistant turn, which can break reply generation, attachment extraction, and message-kind shaping.
