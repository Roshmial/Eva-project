...
**Before you install tooling, verify the cheapest path.** For browser-automation or scraping spikes, inspect what the plain HTTP response already contains before committing to Playwright/Selenium. Many modern apps render enough data into initial HTML, hydration payloads, or server components that you can validate the idea without a real browser. Treat this as a required elimination step: first check raw HTML / embedded JSON / public search results, then escalate to browser setup only if the data is genuinely absent from the response.
