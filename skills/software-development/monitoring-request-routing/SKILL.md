...
6. Если monitoring intent уже явный и в user-message есть cadence/schedule (`ежедневно`, `еженедельно`, `в 09:00`, `по пятницам` и т.п.), route создания recurring job должен иметь приоритет над collection-clarification. Иначе runtime легко уходит в ложный `data_collection_clarification`, хотя пользователь просил именно регулярную задачу.
7. Если пользователь дал setup в смешанной форме — одновременно тему мониторинга, sources и расписание — не требовать сначала отдельный one-shot collection-contract. Сначала создать или переиспользовать recurring job, а уже затем при необходимости отдельно улучшать source manifest или первый run.
8. Если источник требует отдельного collector pipeline, recurring job должен создавать/переиспользовать именно этот pipeline, а не вручную пересобираться в тексте ответа.
9. После перевода production workflow в n8n Hermes-версия должна оставаться только в dev/test и без активных рабочих расписаний.
