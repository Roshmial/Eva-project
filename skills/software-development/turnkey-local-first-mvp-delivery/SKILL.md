...
- Letting complete collection requests stop at a parsed contract. If the user asked to actually collect or configure collection, either execute the downstream path (for example Telegram task-specific list + `TG-API /export`) or return an explicit execution-plan artifact with honest source coverage.
