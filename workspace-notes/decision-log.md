[2026-09-15 UTC] — Юнит-экономика РСМ строится как компактная проектно-ресурсная план-факт модель

Context:
- План продаж ведётся агрегированно по каталожным позициям и месяцам; факт — по конкретным договорам, один Project_ID на запись.
- Нужны экономика каждого проекта, направлений и общий P&L; человеческие ресурсы учитываются через FTE и целое количество сотрудников.

Decision:
- Книга ограничена 9 связанными листами: навигация, каталог, план-факт продаж, факт проектов, нормативы, ресурсы, накладные, экономика, контроль.
- Каталог содержит сервисы в размерах S/M/L/XL; плановая потребность формируется из плана продаж, нормативов длительности, ресурсов и месячного профиля.
- Факт проекта хранит продажу, сроки, проданный ресурсный состав, фактическое месячное потребление и EAC.
- Полная стоимость целого ресурсного пула распределяется по проектам пропорционально потреблению; нераспределённый остаток уменьшает прибыль компании.
- Маржа проекта и направления = признанная выручка минус прямые затраты; накладные вычитаются общим пулом только в P&L компании.

Boundary:
- Текущий план 171 единица перенесён как рабочее допущение, а не подтверждённый коммерческий план.
- Фактические данные не синтезируются; реестр подготовлен для ввода или импорта.

[2026-09-11 UTC] — Gamma deck refinements are slide-level edits, not repeated full regenerations

Context:
- Пользователь отметил, что повторная перерисовка презентации с нуля расходует много токенов и плодит версии.

Decision:
- Для утверждённой Gamma-презентации изменять только явно названные слайды и элементы.
- Полную генерацию использовать только при явном запросе на новый сторилайн или новую визуальную концепцию.
- Сохранять не более двух активных версий; архивировать только после проверки замены.

Boundary:
- Исходные Kafka-приложения, которые требуется сохранить без редизайна, не менять.

Content framework:
- Сторилайн презентации: управленческий приоритет и цена бездействия → рычаги ценности → четыре ситуации принятия решений → возможности ресурсно-сервисного управления и финансовой модели TBM → зрелость → реализация и масштабирование ценности → пилот и владельцы результата.
- Денежный эффект показывать через базовый и целевой сценарии, с владельцем, сроком и последующей проверкой факта; потенциальный эффект не считать реализованным.
- В иллюстративной траектории из ITFM-референса на этапе 3 «Внутренняя цена» у ИТ-функции появляется управленческий финансовый результат (P&L) как разница между тарифами и себестоимостью; на этапе 4 «Внешняя цена» он сохраняется, а тарифы сопоставляются с рынком. Эта траектория не является стандартной шкалой зрелости TBM/FinOps; применимость P&L зависит от модели внутренних расчётов и учётной политики.
- Целевая логика эффекта: пилот подтверждает потенциал оптимизации от 7% бюджета инфраструктуры; развитый контур с внутренними тарифами, управлением потреблением и управленческим P&L может расширить потенциал до 15%. Значения не суммируются, зависят от периметра и данных и не являются гарантией результата.
- Слайд ролей показывает несколько выгод для каждой роли — по три практические возможности для CIO, CFO, директора по инфраструктуре и бизнеса. Роли нельзя сужать до одного кейса или одного решения; обзор сценариев отделён от ролевых выгод.
